...
| Agentic RAG (Python)   | | (Python)               | | (Python / FFmpeg)      | | Worker (Python)     |
+------------+-----------+ +------------+-----------+ +------------+-----------+ +---------+---------+
             |                          |                          |                       |
             +--------------------------+--------------------------+-----------------------+
                                                   |
...
*   **Generative AI & Agentic RAG Engine:** Nhận Job từ Message Broker, gọi API sang LLMs (OpenAI/Anthropic) hoặc external AI (ElevenLabs/Veo 3). Đặc biệt, hệ thống dùng mô hình Agentic RAG để truy vấn chéo giữa SQL DB (lịch sử sales, GMV từng SKU) và Vector DB (guide bán hàng, script cũ) nhằm tự động gợi ý host/khung giờ tối ưu và sinh kịch bản ROS thông minh.
...
### D. Agentic RAG Flow (Smarter Scheduling & Scripting)
1. User chọn tập SKU cần bán (ví dụ: Kem chống nắng, Sữa rửa mặt) và chuẩn bị lên lịch live.
2. **Resource & Scheduling Service** gọi sang **Agentic RAG Engine**.
3. RAG Agent query **Amazon RDS** lấy lịch sử bán chạy theo giờ và GMV/CR (Conversion Rate) của các Host đã từng bán các nhóm SKU này.
4. RAG Agent query **Amazon OpenSearch** lấy các USP/Script cũ đã từng chốt sale tốt.
5. AI tổng hợp lại và trả về kết quả: "Nên xếp Host A vào khung 20h vì Host A có tỷ lệ chốt Kem chống nắng tốt nhất. Script tập trung nhấn mạnh vào độ mỏng nhẹ (dựa trên feedback cũ)". Hệ thống sau đó tự động điền các thông tin này vào Smart Calendar và ROS.
